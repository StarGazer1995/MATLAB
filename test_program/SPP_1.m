...
function  [sta_xyz_clk,DOP,num_sat]= SPP_1(STA_name,Rinex_o,Rinex_n,YUMA,x0,x_true,cutoff,file);
...
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%end
        traval_time=0.07;
        traval_time_0=1;
        while abs(traval_time_0-traval_time)>=0.00000001
            traval_time_0=traval_time;
            tx_GPS=nobs(i,2)-traval_time_0;
            dt = tx_GPS-toc;
            if dt >  302400 
                dt = dt-2*302400;
            end
            ...
            [E] = keplr_eq(gps_ephem(2),gps_ephem(4));
            rele_cor=-4.442807633*10^(-10)*gps_ephem(4)*gps_ephem(5)*sin(E); %%reletive correction
            tcorr=tcorr+rele_cor;
            tcorr=tcorr-gps_GDC;   %%%%%%%% GDC is the Group delay correction
            tx_GPS=tx_GPS-tcorr;
            temp_t=[t_start(i,1) tx_GPS];
            [t_out, prn, x, v] = propgeph(gps_ephem, temp_t);
            traval_time=sqrt(sum((x-x0).^2))/C;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%% End of changing TOT determination
        nobs(i,5) = tx_GPS;
