function textwin2(fig_title_cell,x,y,prn,axis_label_cell,legend_cell, ...
...
if nargin<8,  % No done flag provided
  done_flag = 0;
end;
...
set(gca,'box','on');
plot_svn([x zeros(size(x,1),1)],y,prn);
if ~isempty(axis_label_cell(3,:)),
  title(axis_label_cell(3,:));
end;
% no x-axis label, that is automatically done in plot_svn
if ~isempty(axis_label_cell(2,:)),
  ylabel(axis_label_cell(2,:));
end;

% don't add a legend to the plot_svn plots.  This is already done with the
% labeling with the satellite numbers
