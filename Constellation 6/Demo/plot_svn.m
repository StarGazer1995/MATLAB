function p_handle = plot_svn(t,plot_data,prn,y_label,plot_title)

% p_handle = plot_svn(t,plot_data,prn,y_label,plot_title);
%
...
% labeled with the prn number. If all the prn numbers are 0, then no prn
% numbers will be added to the plot. This function will use the current figure
...
%                Include a rollover_flag of 0 for times prior to
%                August 22, 1999.
%   plot_data  - data to be plotted (nx1)
%   prn        - satellite number, nx1 (optional), 
%                if not provided, a single line is drawn
%   y_label    - label for the y-axis, (1xk string) (optional) 
%                default = 'Y Data'
%   plot_title - title of plot (1xm string) (optional) default = 'Data Plot'
% Output:
...
if ~isempty(msg)
  fprintf('%s  See help on PLOT_SVN for details.\n',msg);
...
if matlab_version >= 5.0                        
  ...
  estruct.variable(2).var = plot_data;
...
for i = 1:num_prns               % loop over the active satellites
  ...
  % plot the plot_data/plot_data pairs for this satellite
  plot(t_plot(I),plot_data(I),'Color',label_color);
  ...
  if active_sats(i) ~= 0,  
    if any(I_mid)
      qts = text(t_plot(I(I_mid)),plot_data(I(I_mid)),num2str(active_sats(i)), ...
                 'Color',label_color);         % put the label on
      clear I_mid
    elseif any(I_end)
      qts = text(t_plot(I(I_end)),plot_data(I(I_end)),num2str(active_sats(i)), ...
                 'Color',label_color);         % put the label on
      clear I_end
    elseif any(I_beg)
      qts = text(t_plot(I(I_beg)),plot_data(I(I_beg)),num2str(active_sats(i)), ...
                 'Color',label_color);         % put the label on
      clear I_beg
    end % if any(I_mid)
